...
class NaviGrabIntegration : public content::WebContentsObserver {
 public:
  ...
  // Memory-based screenshot methods for tooltips
  void CaptureElementThumbnail(const ElementInfo& element_info,
                              base::OnceCallback<void(const std::vector<uint8_t>&)> callback);
  
  void CapturePageThumbnail(base::OnceCallback<void(const std::vector<uint8_t>&)> callback);
  ...
  void PerformFreshCrawl(base::OnceCallback<void(bool)> callback);
