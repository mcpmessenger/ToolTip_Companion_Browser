...
void TooltipService::PerformFreshCrawl(base::OnceCallback<void(bool)> callback) {
  if (!initialized_ || !navigrab_integration_) {
    std::move(callback).Run(false);
    return;
  }
  
  VLOG(1) << "Performing fresh crawl via TooltipService";
  
  // Delegate to NaviGrab integration
  navigrab_integration_->PerformFreshCrawl(std::move(callback));
}
