...
void NaviGrabIntegration::InitializeNaviGrab() {
  ...
  if (!core.Initialize()) {
    LOG(ERROR) << "Failed to initialize NaviGrab core";
    return;
  }
  
  // Create NaviGrab components using factory functions
  web_automation_ = navigrab::CreateWebAutomation();
  browser_ = navigrab::CreateBrowser();
  page_ = navigrab::CreatePage();
  screenshot_capture_ = navigrab::CreateScreenshotCapture();
  
  LOG(INFO) << "NaviGrab components created successfully with factory functions";
}
...
void NaviGrabIntegration::StartMonitoring(content::WebContents* web_contents) {
  ...
  if (monitoring_ && web_contents == this->web_contents()) {
...
void NaviGrabIntegration::ExecuteAction(
  ...
  // base::TimeTicks start_time = base::TimeTicks::Now(); // TODO: Add timing
...
void NaviGrabIntegration::ClickElement(
  ...
  bool success = page_->Click(selector);
...
void NaviGrabIntegration::TypeText(
  ...
  bool success = page_->Type(selector, text);
  AutomationResult result = ProcessResult(success,
...
void NaviGrabIntegration::HoverElement(
  ...
  bool success = page_->Hover(selector);
  AutomationResult result = ProcessResult(success,
...
void NaviGrabIntegration::CapturePageScreenshot(
  ...
  bool success = screenshot_capture_->CaptureFullPage(filename);
  AutomationResult result = ProcessResult(success,
                                        success ? filename : "",
...
void NaviGrabIntegration::CaptureElementThumbnail(
    const ElementInfo& element_info,
  ...
  // Capture element data to memory
  std::vector<uint8_t> element_data = screenshot_capture_->CaptureElementData(selector);
  
  // Generate thumbnail for tooltip display
  std::vector<uint8_t> thumbnail = screenshot_capture_->GenerateThumbnail(element_data, 200, 150);
  
  std::move(callback).Run(thumbnail);
}

void NaviGrabIntegration::CapturePageThumbnail(
    base::OnceCallback<void(const std::vector<uint8_t>&)> callback) {
  ...
  // Capture page data to memory
  std::vector<uint8_t> page_data = screenshot_capture_->CapturePageData();
  
  // Generate thumbnail for tooltip display
  std::vector<uint8_t> thumbnail = screenshot_capture_->GenerateThumbnail(page_data, 200, 150);
...
void NaviGrabIntegration::PerformFreshCrawl(base::OnceCallback<void(bool)> callback) {
  if (!initialized_ || !enabled_) {
    std::move(callback).Run(false);
    return;
  }
  
  LOG(INFO) << "Performing fresh crawl of current page";
  
  // Re-initialize NaviGrab components for fresh state
  InitializeNaviGrab();
  ...
  if (web_automation_) {
    auto interactive_elements = web_automation_->DiscoverInteractiveElements();
    auto form_elements = web_automation_->DiscoverFormElements();
    auto nav_elements = web_automation_->DiscoverNavigationElements();
    
    LOG(INFO) << "Fresh crawl discovered " << interactive_elements.size() 
              << " interactive elements, " << form_elements.size() 
              << " form elements, " << nav_elements.size() 
              << " navigation elements";
  }
